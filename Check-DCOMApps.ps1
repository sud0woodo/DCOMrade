...
If the rule is not present it is added with netsh:
...
function Get-Vulnerable($remotesession) {
    ...
    Invoke-Command -Session $remotesession -ScriptBlock {Get-ChildItem -Path HKCR:\AppID\ | ForEach-Object {if($_.Property -Match "LaunchPermission") {Continue} else {$_.Name.Replace("HKEY_CLASSES_ROOT\AppID\","")}}} -OutVariable DefaultPermissionsAppID | Out-Null 
    $DCOMApplications = Invoke-Command -Session $remotesession -ScriptBlock {Get-CimInstance Win32_DCOMapplication}
    $PossibleVulnerable = $DCOMApplications | Select-String -Pattern $DefaultPermissionsAppID
    Write-Host "[+] Found $($PossibleVulnerable.Count) possibly vulnerable DCOM applications" -ForegroundColor Green
    $PossibleVulnerable
}
